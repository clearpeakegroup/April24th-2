...
load_dotenv()
MARKETWATCH_API_URL = os.getenv("MARKETWATCH_API_URL", "https://www.marketwatch.com/news/latest-news")
RAW_BASE = "data/lake/raw/news"
POLL_INTERVAL = int(os.getenv("MARKETWATCH_POLL_SEC", 10))

async def ingest_marketwatch_news():
    last_seen = set()
    while True:
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.get(MARKETWATCH_API_URL)
                ...
                for item in news_items:
                    ...
                    fname = f"{partition}/marketwatch_{now.strftime('%H%M%S_%f')}.json.zst"
                    import zstandard as zstd
                    cctx = zstd.ZstdCompressor()
                    with open(fname, "wb") as f:
                        f.write(cctx.compress(json.dumps(item).encode('utf-8')))
                    logger.info(f"Saved MarketWatch news: {fname}")
                    last_seen.add(news_id)
        except Exception as e:
            logger.error(f"MarketWatch ingest error: {e}")
