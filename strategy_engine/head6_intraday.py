...
class StrategyHead(BaseStrategy):
    def backtest(self, data: pd.DataFrame) -> BacktestResult:
        # Stub: Buy if price below mean, sell if above
        trades = []
        equity_curve = []
        cash = 100000
        pos = 0
        ...
        for i, row in data.iterrows():
            if row['NQ'] < mean and pos == 0:
                price = apply_slippage(row['NQ'])
                trades.append({'side': 'buy', 'qty': 1, 'price': price})
                pos = 1
                cash -= price
            elif row['NQ'] > mean and pos == 1:
                price = apply_slippage(row['NQ'])
                trades.append({'side': 'sell', 'qty': 1, 'price': price})
                pos = 0
                cash += price
            equity_curve.append(cash + (row['NQ'] if pos else 0))
        pnl = calc_pnl(trades, data['NQ'])
    ...
    async def live(self, feed: AsyncIterable) -> AsyncIterator[Order]:
        async for row in feed:
            yield Order(symbol='NQ', qty=1, price=row['NQ'], side='buy') 